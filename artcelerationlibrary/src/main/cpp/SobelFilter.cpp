                ...
                for (x_iterator = 0; x_iterator < total_width; x_iterator++){
                    ...
                    for(int i= -1; i<2;i++){
                        for(int j=-1; j<2;j++){
                            ...
                            if(j==0 || (y_iterator+i) <0 || (y_iterator+i>=total_height)|| (x_iterator+j)<0 || (x_iterator+j)>=total_width){
                                continue;
                            }
                            sum_x += gray[((y_iterator+i)*total_width)+x_iterator+j]*Sx_mask[1+i][1+j];
                        }
                    }
                    ...
                    input_line[y_iterator*total_width+ x_iterator] = ((sum_x << 16) & 0x00FF0000) |
                                                                     ((sum_x << 8) & 0x0000FF00) |
                                                                     (sum_x & 0x000000FF);
                }
            }
                ...
                for (y_iterator = 0; y_iterator < total_height; y_iterator++) {
                    ...
                    for (x_iterator = 0; x_iterator < total_width; x_iterator++){
                        ...
                        for(int i= -1; i<2;i++){
                            for(int j=-1; j<2;j++){
                                //Skip the pixels that go out of range or where the mask is 0
                                if(i==0 || (y_iterator+i) <0 || (y_iterator+i>=total_height)|| (x_iterator+j)<0 || (x_iterator+j)>=total_width){
                                    continue;
                                }
                                sum_y += gray[((y_iterator+i)*total_width)+x_iterator+j]*Sy_mask[1+i][1+j];
                            }
                        }
                        //Limit the values between 0 and 255
                        ...
                        //Insert the updated value into the pixel
                        input_line[y_iterator*total_width+ x_iterator] = (((int)sum_y << 16) & 0x00FF0000) |
            ...
            case 2:
                ...
                for (y_iterator = 0; y_iterator < total_height; y_iterator++) {
                    ...
                    for (x_iterator = 0; x_iterator < total_width; x_iterator++){
                        ...
                        //Insert the updated value into the pixel
                        input_line[y_iterator*total_width+ x_iterator] = (((int)out << 16) & 0x00FF0000) |
